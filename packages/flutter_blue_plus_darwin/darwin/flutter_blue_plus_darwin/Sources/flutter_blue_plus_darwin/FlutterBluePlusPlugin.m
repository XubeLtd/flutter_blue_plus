...
@property(nonatomic)
    NSMutableDictionary<NSString *, CBL2CAPChannel *> *l2capChannels;
@property(nonatomic) FlutterResult pendingL2capResult;
@property(nonatomic) NSString *pendingSocketId;
...
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar> *)registrar {
  ...
  instance.l2capChannels = [NSMutableDictionary dictionary];
...
- (void)handleMethodCall:(FlutterMethodCall *)call
  ...
  @try {
    ...
    if ([@"getAdapterState" isEqualToString:call.method]) {
      ...
      result(@YES);
    } else if ([@"createL2CapChannel" isEqualToString:call.method]) {
      NSLog(@"createL2CapChannel");
      NSString *remoteId = call.arguments[@"remote_id"];
      NSNumber *psm = call.arguments[@"psm"];

      CBPeripheral *peripheral = self.connectedPeripherals[remoteId];
      if (peripheral == nil) {
      ...
      NSLog(@"[FBP-iOS] Creating L2CAP channel for PSM: %d", psm.intValue);
      // log self.l2capChannels
      NSLog(@"[FBP-iOS] L2CAP channels: %@", self.l2capChannels);

      // Check if any L2CAP channel with this PSM is already open
      for (NSString *key in self.l2capChannels) {
        CBL2CAPChannel *channel = self.l2capChannels[key];
        if (channel.PSM == psm.unsignedShortValue) {
          result([FlutterError
              errorWithCode:@"L2CAP_ALREADY_OPEN"
                    message:@"L2CAP channel already open for this PSM"
                    details:nil]);
          return;
        }
      }
      ...
      // Store callback and generated socket ID
      self.pendingL2capResult = result;
      self.pendingSocketId =
          [[NSUUID UUID] UUIDString]; // Auto-generated socket ID
    } else if ([@"l2CapRead" isEqualToString:call.method]) {
      NSLog(@"l2CapRead");
      NSString *socketId = call.arguments[@"socket_id"];
      NSLog(@"[FBP-iOS] Reading from L2CAP channel with socket ID: %@",
            socketId);
      CBL2CAPChannel *channel = self.l2capChannels[socketId];
      NSLog(@"[FBP-iOS] L2CAP channel: %@", channel);
      if (channel) {
        NSInputStream *inputStream = channel.inputStream;

        if (inputStream.streamStatus != NSStreamStatusOpen &&
            inputStream.streamStatus != NSStreamStatusReading) {
          result([FlutterError errorWithCode:@"L2CAP_STREAM_CLOSED"
                                     message:@"Input stream is not open"
                                     details:nil]);
          return;
        }
        ...
        NSLog(@"1.[FBP-iOS] Read %ld bytes from socket ID: %@", (long)bytesRead,
              socketId);
        if (bytesRead > 0) {
          NSLog(@"2.[FBP-iOS] Read %ld bytes from socket ID: %@",
                (long)bytesRead, socketId);
          NSData *data = [NSData dataWithBytes:buffer length:bytesRead];
          NSLog(@"3.[FBP-iOS] Read %ld bytes from socket ID: %@",
                (long)bytesRead, socketId);
          result(data);
        } else if (bytesRead == 0) {
          NSLog(@"[FBP-iOS] No data available at this moment");
          result([FlutterError errorWithCode:@"L2CAP_NO_DATA"
                                     message:@"No data available at this moment"
                                     details:nil]);
        } else {
          result([FlutterError
              errorWithCode:@"L2CAP_READ_ERROR"
                    message:@"Error while reading from L2CAP channel"
                    details:nil]);
        }

      } else {
        result([FlutterError errorWithCode:@"INVALID_SOCKET"
                                   message:@"Socket not found"
                                   details:nil]);
      }
    } else if ([@"l2CapWrite" isEqualToString:call.method]) {
      NSLog(@"l2CapWrite");
      NSString *socketId = call.arguments[@"socket_id"];
      ...
      CBL2CAPChannel *channel = self.l2capChannels[socketId];
      if (channel) {
        NSOutputStream *outputStream = channel.outputStream;
        NSInteger written = [outputStream write:data.bytes
                                      maxLength:data.length];

        if (written > 0) {
          result(@(YES));
        } else {
          result([FlutterError errorWithCode:@"L2CAP_WRITE_ERROR"
      ...
      NSLog(@"closeL2CapChannel");
      NSString *socketId = call.arguments[@"socket_id"];
      CBL2CAPChannel *channel = self.l2capChannels[socketId];
      NSLog(@"[FBP-iOS] Closing L2CAP channel with socket ID: %@", socketId);
      if (channel) {
        [channel.inputStream close];
        [channel.outputStream close];
        [self.l2capChannels removeObjectForKey:socketId];
        result(@(YES));
      } else {
        result([FlutterError errorWithCode:@"INVALID_SOCKET"
                                   message:@"Socket not found"
...
//     NSLog(@"[FBP-iOS] Failed to open L2CAP channel: %@", error);
//     if (self.pendingL2capResult) {
//       self.pendingL2capResult([FlutterError
//           errorWithCode:@"L2CAP_CHANNEL_ERROR"
//                 message:@"Failed to open L2CAP channel"
//                 details:error.localizedDescription]);
//     }
//     self.pendingL2capResult = nil;
//     self.pendingSocketId = nil;
//     return;
//   }

//   // Store channel
//   self.l2capChannels[self.pendingSocketId] = channel;

//   // Return socket ID
//   NSLog(@"[FBP-iOS] Opened L2CAP channel with socketId: %@",
//         self.pendingSocketId);
//   if (self.pendingL2capResult) {
//     self.pendingL2capResult(self.pendingSocketId);
//   }
...
- (void)peripheral:(CBPeripheral *)peripheral
    didOpenL2CAPChannel:(CBL2CAPChannel *)channel
                  error:(NSError *)error {
  if (error) {
    NSLog(@"[FBP-iOS] Failed to open L2CAP channel: %@",
          error.localizedDescription);
    if (self.pendingL2capResult) {
      self.pendingL2capResult([FlutterError
          errorWithCode:@"L2CAP_OPEN_FAILED"
                message:error.localizedDescription
                details:nil]);
      self.pendingL2capResult = nil;
    }
  ...
  NSLog(@"[FBP-iOS] L2CAP channel opened: %@", channel);

  // Store channel using the previously generated socket ID
  if (self.pendingSocketId != nil) {
    self.l2capChannels[self.pendingSocketId] = channel;
    if (self.pendingL2capResult) {
      self.pendingL2capResult(self.pendingSocketId);
    }
    ...
    NSLog(@"[FBP-iOS] Warning: No pending socket ID when channel opened.");
  }

  // Schedule input/output streams
  [channel.inputStream scheduleInRunLoop:[NSRunLoop mainRunLoop]
                                 forMode:NSDefaultRunLoopMode];
  [channel.inputStream open];
  [channel.outputStream scheduleInRunLoop:[NSRunLoop mainRunLoop]
                                  forMode:NSDefaultRunLoopMode];
...
- (void)peripheral:(CBPeripheral *)peripheral
  ...
  NSString *key = [NSString stringWithFormat:@"%@:%@:%@:%@", remoteId,
                                             ...
                                             primaryService != nil
                                                 ...
                                                 : @""];
  NSData *value =
      self.writeChrs[key] ? self.writeChrs[key] : [NSMutableData data];
