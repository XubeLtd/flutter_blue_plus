...
import com.lib.flutter_blue_plus.FlutterBluePlusPlugin.LogLevel;
...
public class FlutterBluePlusPlugin implements
    ...
    private final Map<String, BluetoothSocket> l2capSockets = new HashMap<>();
    ...
    public void onMethodCall(@NonNull MethodCall call,
        ...
        try {
            ...
            switch (call.method) {
                        ...
                        if (Build.VERSION.SDK_INT >= 23) { // Android 6.0 (October 2015)
                            ...
                            try {
                                ...
                                Method connectGattMethod = device.getClass()
                                        .getDeclaredMethod("connectGatt", Context.class, boolean.class,
                                                BluetoothGattCallback.class, int.class);
                                connectGattMethod.setAccessible(true);
                                gatt = (BluetoothGatt) connectGattMethod.invoke(device, context, autoConnect,
                                        mGattCallback, 2 /* TRANSPORT_LE */);
                            } catch (Exception ex) {
                ...
                // Create l2cap channel then return the bluetooth socket
                case "createL2CapChannel": {
                    Map<String, Object> args = (Map<String, Object>) call.arguments;
                    String remoteId = (String) args.get("remote_id");
                    int psm = (int) args.get("psm");

                    BluetoothDevice device = mBluetoothAdapter.getRemoteDevice(remoteId);

                    try {
                        BluetoothSocket socket = device.createInsecureL2capChannel(psm);
                        socket.connect(); // don't forget to connect!

                        String socketId = UUID.randomUUID().toString();
                        l2capSockets.put(socketId, socket);

                        result.success(socketId); // return socket ID to Dart
                    } catch (IOException e) {
                        result.error("L2CAP_CONNECT_ERROR", "Failed to create/connect L2CAP channel", e.getMessage());
                    }

                    break;
                }

                // Destroy or close the l2cap channel
                case "closeL2CapChannel": {
                    ...
                    if (socket != null) {
                        try {
                            socket.close();
                            l2capSockets.remove(socketId);
                            result.success(true);
                        } catch (IOException e) {
                            result.error("L2CAP_CLOSE_ERROR", "Close failed", e.getMessage());
                        }
                    } else {
                ...
                case "l2CapRead": {
                    ...
                    if (socket != null) {
                        try {
                            ...
                            byte[] buffer = new byte[1024]; // adjust as needed
                            int bytesRead = inputStream.read(buffer);
                            if (bytesRead != -1) {
                                byte[] resultBytes = Arrays.copyOf(buffer, bytesRead);
                                // print the result
                                log(LogLevel.DEBUG, "Read " + bytesRead + " bytes: " + Arrays.toString(resultBytes));
                                result.success(resultBytes);
                            } else {
                                result.success(new byte[0]);
                            }
                        } catch (IOException e) {
                            result.error("L2CAP_READ_ERROR", "Read failed", e.getMessage());
                        }
                    } else {
                        result.error("INVALID_SOCKET", "Socket not found", null);
                    }
                ...
                case "writeCharacteristic": {
                    ...
                    int writeType = writeTypeInt == 0 ? BluetoothGattCharacteristic.WRITE_TYPE_DEFAULT
                    ...
                    if (dataLen > maxLen) {
                        ...
                        String str = "data longer than allowed. dataLen: " + dataLen + " > max: " + maxLen + " (" + a
                                + b + ")";
                ...
                case "readDescriptor": {
                    ...
                    // find descriptor
                    BluetoothGattDescriptor descriptor = getDescriptorFromArray(descriptorUuid,
                            characteristic.getDescriptors());
                    if (descriptor == null) {
                        String s = "descriptor not found on characteristic. (desc: " + descriptorUuid + " chr: "
                                + characteristicUuid + ")";
                ...
                case "setNotifyValue": {
                    ...
                    if (cccd == null) {
                        ...
                        // thus setCharacteristicNotification() is all that is required to enable
                        // notifications.
                ...
                case "requestConnectionPriority": {
                    ...
                    if (gatt.requestConnectionPriority(cpInteger) == false) {
                        result.error("requestConnectionPriority", "gatt.requestConnectionPriority() returned false",
                                null);
                        break;
                    }
    ...
    private boolean isLocationEnabled() {
        if (Build.VERSION.SDK_INT >= 31) {
            // Android 12 (October 2021) - location services are not required for BLE
            // scanning
            return true;
        }
    ...
    private ChrFound locateCharacteristic(BluetoothGatt gatt,
        ...
        if (isSecondaryService) {
            ...
            if (secondaryService == null) {
                return new ChrFound(null, "secondary service not found '" + serviceUuid + "' (primary service '"
                        + primaryServiceUuid + "')");
            }
        }
        ...
        // characteristic
        BluetoothGattCharacteristic characteristic = getCharacteristicFromArray(characteristicUuid,
    ...
    private BluetoothGattCharacteristic getCharacteristicFromArray(String uuid,
            List<BluetoothGattCharacteristic> array) {
    ...
    private int getMaxPayload(String remoteId, int writeType, boolean allowLongWrite) {
        ...
        // be longer than 512. Android also enforces this as the maximum in internal
        // code.
        int maxAttrLen = 512;

        // if no response, or allowLongWrite is not allowed, we can only write up to
        // MTU-3.
    ...
    byte[] getManufacturerSpecificData(ScanRecord adv) {
        ...
        while (n < bytes.length) {
            ...
            // and that the field is large enough (at least 2 bytes: type + at least 1 data
            // byte)
    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            try {
                ...
                // android never uses this callback with enums values of CONNECTING or
                // DISCONNECTING,
        ...
        // Android has an annoying edge case. If disconnect is called right as the
        // connection is being
        // established, Android sometimes ignores the request to disconnect and
        // completes the connection
        // anyway. To handle this case, we make sure the device is still in our
        // currently connecting
        // devices map, otherwise kill the connection since the user was not expecting
        // it to connect.
        ...
        public void onCharacteristicReceived(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic,
        ...
        public void onCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
            ...
            // For "writeWithResponse", onCharacteristicWrite is called after the remote
            // sends back a write response.
            // For "writeWithoutResponse", onCharacteristicWrite is called as long as there
            // is still space left
            // in android's internal buffer. When the buffer is full, it delays calling
        ...
        public void onCharacteristicChanged(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
            // getValue() was deprecated in API level 33 because the function makes it look
            // like
            // you could always call getValue on a characteristic. But in reality,
            // getValue()
    ...
    HashMap<String, Object> bmScanAdvertisement(BluetoothDevice device, ScanResult result) {
        ...
        if (Build.VERSION.SDK_INT >= 26) { // Android 8.0, August 2017
            ...
            // returns if the device wants to be *discoverable*, which is not the same
            // thing.
            connectable = true;
        }
    ...
    HashMap<String, Object> bmBluetoothCharacteristic(BluetoothDevice device,
    ...
    private static String gattErrorString(int value) {
        switch (value) {
            ...
            case 0x80:
                return "GATT_NO_RESOURCES"; // 128
            case 0x81:
                return "GATT_INTERNAL_ERROR"; // 129
            case 0x82:
                return "GATT_WRONG_STATE"; // 130
            case 0x83:
                return "GATT_DB_FULL"; // 131
            case 0x84:
                return "GATT_BUSY"; // 132
            case 0x85:
                return "GATT_ERROR"; // 133
            case 0x86:
                return "GATT_CMD_STARTED"; // 134
            case 0x87:
                return "GATT_ILLEGAL_PARAMETER"; // 135
            case 0x88:
                return "GATT_PENDING"; // 136
            case 0x89:
                return "GATT_AUTH_FAIL"; // 137
            case 0x8a:
                return "GATT_MORE"; // 138
            case 0x8b:
                return "GATT_INVALID_CFG"; // 139
            case 0x8c:
                return "GATT_SERVICE_STARTED"; // 140
            case 0x8d:
                return "GATT_ENCRYPTED_NO_MITM"; // 141
            case 0x8e:
                return "GATT_NOT_ENCRYPTED"; // 142
